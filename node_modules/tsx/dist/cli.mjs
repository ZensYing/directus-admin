...
`));const e={eval:{type:String,alias:"e"},print:{type:String,alias:"p"}},{_:u,flags:s}=zu({flags:{...e,inputType:String,test:Boolean},help:!1,ignoreArgv:lt(!1)}),n=wn({...vn,...e}),i=["print","eval"].find(c=>!!s[c]);if(i){const{inputType:c}=s,f=s[i],h=xn(f,{loader:"default",sourcefile:"/eval.ts",format:c==="module"?"esm":"cjs"});n.unshift(`--${i}`,h.code)}wu(Pn)&&s.test&&u.length===0&&n.push("**/{test,test/**/*,test-*,*[.-_]test}.?(c|m)@(t|j)s");const o=await Rn(),D=as(n,{noCache:!!t.flags.noCache,tsconfigPath:t.flags.tsconfig});Bl(D,o),process.send&&D.on("message",c=>{process.send(c)}),D.send&&process.on("message",c=>{D.send(c)}),D.on("close",c=>{c===null&&(c=ht.signals[D.signalCode]+128),process.exit(c)})});
